...
//		}
	...
	public static String getResourceUuidFromName(String resourceName) {
...
//		return reference.getUUID();
	...
	public static MADAttachmentReference getAttachmentReference(int fileHandlerId, int id, int attachmentId, String resourceName, int imageId, int archiveId) {
		MADAttachmentReference resourceReference = null;
		if (id > 0) {
			resourceReference = MADAttachmentReference.getById(Env.getCtx(), id, null);
		}
		...
		if (resourceReference == null && !Util.isEmpty(resourceName, true)) {
	 ...
	 * @throws Exception 
	 */
	private void getResource(int resourceId, int attachmentId, String resourceName, int imageId, int archiveId,
		StreamObserver<Resource> responseObserver) throws Exception {
...
//		MADAttachmentReference resourceReference = getAttachmentReference(
//			clientInfo.getFileHandler_ID(),
//			request.getId(),
//			request.getAttachmentId(),
//			request.getResourceName(),
...
//		if (!MimeType.isValidMimeType(fileName)) {// TODO core
...
//			attachmentReference.setFileHandler_ID(clientInfo.getFileHandler_ID());
			...
			if(request.getResourceType() == ResourceType.IMAGE) {
				...
				MImage image = new MImage(Env.getCtx(), request.getId(), transactionName);
				if (image == null) {
...
//				}
				...
				if (attachment.getAD_Attachment_ID() <= 0) {
					 ...
					 */
//					attachment.setIsDirectLoad(true);.// TODO core
					attachment.saveEx();
				}
...
//			MADAttachmentReference.resetAttachmentReferenceCache(clientInfo.getFileHandler_ID(), attachmentReference);
	...
	ResourceReference.Builder setResourceReferenceDescription(SetResourceReferenceDescriptionRequest request) {
		...
		if (resourceReference.getAD_Image_ID() > 0) {
			MImage image = MImage.get(Env.getCtx(), resourceReference.getAD_Image_ID());
			image.setDescription(request.getDescription());
			image.saveEx();
...
//
//		// reset cache
//		MADAttachmentReference.resetAttachmentReferenceCache(
//			clientInfo.getFileHandler_ID(),
//			resourceReference
//		);
