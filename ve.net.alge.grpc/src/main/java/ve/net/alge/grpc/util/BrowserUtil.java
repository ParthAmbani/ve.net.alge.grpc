...
public class BrowserUtil {
	...
	public static Map<String, Integer> getBrowseFieldsSelectionDisplayType(MBrowse browser) {
		List<MBrowseField> browseFields = new Query(browser.getCtx(), MBrowseField.Table_Name,
				"AD_Browse_ID = ? AND (IsIdentifier = ? OR IsKey = ? OR IsReadOnly = ?)", null)
				.setOnlyActiveRecords(true).setParameters(browser.getAD_Browse_ID(), true, true, false)
		...
		browseFields.forEach(browseField -> {
			MViewColumn viewColumn = MViewColumn.getById(browser.getCtx(), browseField.getAD_View_Column_ID(), null);

			displayTypeColumns.put(viewColumn.getColumnName(), browseField.getAD_Reference_ID());
		});
