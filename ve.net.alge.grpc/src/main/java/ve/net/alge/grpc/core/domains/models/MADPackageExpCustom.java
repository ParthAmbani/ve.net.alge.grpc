...
public class MADPackageExpCustom extends X_AD_Package_Exp_Custom {
	...
	/** Static Cache */
	private static CCache<Integer, MADPackageExpCustom> exporterCacheIds = new CCache<Integer, MADPackageExpCustom>(
			Table_Name, 30);
	/** Static Cache */
	private static CCache<String, MADPackageExpCustom> exporterCacheValues = new CCache<String, MADPackageExpCustom>(
			Table_Name, 30);
	...
	public static MADPackageExpCustom getById(Properties ctx, int definitionId, String trxName) {
		...
		definition = new Query(ctx, Table_Name, COLUMNNAME_AD_Package_Exp_Custom_ID + "=?", trxName)
				.setParameters(definitionId).first();
		if (definition != null && definition.get_ID() > 0) {
	...
	public static List<MADPackageExpCustom> getAll(Properties ctx, boolean resetCache, String trxName) {
		...
		if (resetCache || exporterCacheIds.size() > 0) {
			definitionList = new Query(Env.getCtx(), Table_Name, null, trxName).setOrderBy(COLUMNNAME_Value).list();
		...
		definitionList = exporterCacheIds.entrySet().stream().map(activity -> activity.getValue())
