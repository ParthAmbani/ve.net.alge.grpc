...
import ve.net.alge.grpc.pos.service.POSLogic;
import ve.net.alge.grpc.pos.service.bank.BankManagement;
import ve.net.alge.grpc.pos.service.cash.CashManagement;
import ve.net.alge.grpc.pos.service.cash.CashUtil;
import ve.net.alge.grpc.pos.service.cash.CollectingManagement;
import ve.net.alge.grpc.pos.service.order.OrderManagement;
import ve.net.alge.grpc.pos.service.order.OrderUtil;
import ve.net.alge.grpc.pos.service.order.RMAUtil;
import ve.net.alge.grpc.pos.service.order.ReturnSalesOrder;
import ve.net.alge.grpc.pos.service.order.ReverseSalesTransaction;
import ve.net.alge.grpc.pos.service.pos.POS;
import ve.net.alge.grpc.pos.util.ColumnsAdded;
import ve.net.alge.grpc.pos.util.OrderConverUtil;
import ve.net.alge.grpc.pos.util.POSConvertUtil;
import ve.net.alge.grpc.pos.util.PaymentConvertUtil;
import ve.net.alge.grpc.pos.util.TicketHandler;
import ve.net.alge.grpc.pos.util.TicketResult;
import ve.net.alge.grpc.service.core_functionality.CoreFunctionalityConvert;
import ve.net.alge.grpc.store.model.MCPaymentMethod;
import ve.net.alge.grpc.store.util.VueStoreFrontUtil;
import ve.net.alge.grpc.util.ConvertUtil;
import ve.net.alge.grpc.util.DocumentUtil;
import ve.net.alge.grpc.util.FileUtil;
...
//		.setLimit(limit, offset)
	...
	private Empty.Builder allocateSeller(AllocateSellerRequest request) {
		...
		Trx.run(transactionName -> {
			int[] allocatedSellersIds = new Query(Env.getCtx(), "C_POSSellerAllocation", "C_POS_ID = ?", transactionName).setParameters(posId)
					.getIDs();
			if(!pointOfSales.get_ValueAsBoolean("IsAllowsConcurrentUse")) {
				Arrays.stream(allocatedSellersIds)
...
//			});
			...
			int[] paymentsIdList = new Query(
				...
				.getIDs()
			;
			if(paymentsIdList == null || paymentsIdList.length == 0) {
				throw new AdempiereException("@C_Payment_ID@ @NotFound@");
			}

			Arrays.stream(paymentsIdList).forEach(paymentId -> {
...
//			shipment.setC_POS_ID(salesOrder.getC_POS_ID());// TODO core
			shipment.saveEx(transactionName);
...
//		MBPartner businessPartner = MBPartner.getTemplate(context, clientId, pos.getC_POS_ID());
		MBPartner businessPartner = MBPartner.getTemplate(context, clientId);
...
//		Optional.ofNullable(address.getContactName()).ifPresent(contact -> businessPartnerLocation.setContactPerson(contact));// TODO core
		Optional.ofNullable(address.getLocationName()).ifPresent(locationName -> businessPartnerLocation.setName(locationName));
//		Optional.ofNullable(address.getEmail()).ifPresent(email -> businessPartnerLocation.setEMail(email));// TODO core
		Optional.ofNullable(address.getPhone()).ifPresent(phome -> businessPartnerLocation.setPhone(phome));
//		Optional.ofNullable(address.getDescription()).ifPresent(description -> businessPartnerLocation.setDescription(description));// TODO core
...
//		.setLimit(limit, offset)
	...
	public void configureBPartner(MOrder order, int businessPartnerId, String transactionName) {
		...
		//	Change for payments
		new Query(order.getCtx(),MPayment.Table_Name , MOrder.COLUMNNAME_C_Order_ID + "=?", order.get_TrxName())
		.setClient_ID()
...
//		}
	...
	private Order.Builder createOrder(CreateOrderRequest request) {
		...
		Trx.run(transactionName -> {
			...
			//	Delivery Rules
//			if (pos.getDeliveryRule() != null) {// TODO core
