...
public class RMAUtil {
	...
	public static int getReturnDocumentTypeId(int posId, int sessionPosId, int salesOrderDocumentTypeId) {
		...
		PO documentTypeAllocation = new Query(Env.getCtx(), TABLE_NAME, "C_POS_ID IN(?, ?) AND C_DocType_ID = ?", null)
				.setParameters(pointOfSales.getC_POS_ID(), sessionPosId, salesOrderDocumentTypeId).setClient_ID()
				.setOnlyActiveRecords(true).first();
		int returnDocumentTypeId = 0;
		if (documentTypeAllocation == null || documentTypeAllocation.get_ID() <= 0) {
	...
	public static int getInvoiceLineReferenceId(MOrderLine sourceOrderLine, String transactionName) {
		return new Query(sourceOrderLine.getCtx(), I_C_InvoiceLine.Table_Name, "C_OrderLine_ID = ?", transactionName)
				.setParameters(sourceOrderLine.getC_OrderLine_ID()).setClient_ID().firstId();
	}
	...
	public static MOrder copyRMAFromOrder(MPOS pos, MOrder sourceOrder, String transactionName) {
		...
		PO.copyValues(sourceOrder, returnOrder);
		//
		...
		int targetDocumentTypeId = RMAUtil.getReturnDocumentTypeId(sourceOrder.getC_POS_ID(), pos.getC_POS_ID(),
...
//        	returnOrder.setC_DocTypeTarget_ID(MDocType.getDocTypeBaseOnSubType(sourceOrder.getAD_Org_ID(), // TODO core
//            		MDocType.DOCBASETYPE_SalesOrder , MDocType.DOCSUBTYPESO_ReturnMaterial));
	...
	public static MOrderLine copyRMALineFromOrder(MOrder rma, MOrderLine sourcerOrderLine, String transactionName) {
		...
		returnOrderLine.set_ValueOfColumn(ColumnsAdded.COLUMNNAME_ECA14_Source_OrderLine_ID,
				sourcerOrderLine.getC_OrderLine_ID());
		// Foreign references
		int invoioceLineId = RMAUtil.getInvoiceLineReferenceId(sourcerOrderLine, transactionName);
		if (invoioceLineId > 0) {
//			returnOrderLine.setRef_InvoiceLine_ID(invoioceLineId);// TODO core
		}
		int shipmentLineId = RMAUtil.getShipmentLineReferenceId(sourcerOrderLine, transactionName);
		if (shipmentLineId > 0) {
//			returnOrderLine.setRef_InOutLine_ID(shipmentLineId);// TODO core
		}
	 ...
	 * @param transactionName
	 */
	public static void createReversedPayments(MPOS pos, MOrder sourceOrder, MOrder returnOrder,
			String transactionName) {
		new Query(sourceOrder.getCtx() , MPayment.Table_Name , MOrder.COLUMNNAME_C_Order_ID + "=?", sourceOrder.get_TrxName())
		.setClient_ID()
		.setParameters(sourceOrder.getC_Order_ID())
		.list().forEach(sourcePayment -> {
...
//			returnPayment.addDescription(Msg.parseTranslation(sourceOrder.getCtx(),// TODO core
//					" @From@ " + sourcePayment.getDocumentNo() + " @of@ @C_Order_ID@ " + sourceOrder.getDocumentNo()));
...
//			returnPayment.setIsPrepayment(sourcePayment.isPrepayment());// TODO core
	...
	public static void generateCreditMemoFromRMA(MOrder returnOrder, String transactionName) {
		if (!OrderUtil.isInvoiced(returnOrder.get_ValueAsInt(ColumnsAdded.COLUMNNAME_ECA14_Source_Order_ID),
				transactionName)) {
			return;
		}
		MInvoice invoice = new MInvoice(returnOrder, 0, OrderUtil.getToday());
//    	invoice.setC_POS_ID(returnOrder.getC_POS_ID());// TODO core
		invoice.saveEx();
		// Convert Lines
		int[] returnOrderLineIds = new Query(returnOrder.getCtx(), I_C_OrderLine.Table_Name, "C_Order_ID = ?",
				transactionName).setParameters(returnOrder.getC_Order_ID()).setClient_ID().getIDs();
		for (int returnOrderLineId : returnOrderLineIds) {
	...
	public static void generateReturnFromRMA(MOrder returnOrder, String transactionName) {
		if (!OrderUtil.isDelivered(returnOrder.get_ValueAsInt(ColumnsAdded.COLUMNNAME_ECA14_Source_Order_ID),
				transactionName)) {
			return;
		}
		MInOut shipment = new MInOut(returnOrder, 0, OrderUtil.getToday());
//    	shipment.setC_POS_ID(returnOrder.getC_POS_ID());// TODO core
...
//				returnOrderLine.setRef_InOutLine_ID(shipmentLineId);// TODO core
	...
	public static void copyRMALinesFromOrder(MOrder sourceOrder, MOrder returnOrder, String transactionName) {
		...
		for (int sourceOrderLineId : sourceOrderLineIds) {
			MOrderLine sourcerOrderLine = new MOrderLine(sourceOrder.getCtx(), sourceOrderLineId, transactionName);
			BigDecimal availableQuantity = getAvailableQuantityForReturn(sourcerOrderLine.getC_OrderLine_ID(),
					sourcerOrderLine.getQtyEntered(), sourcerOrderLine.getQtyEntered());
			if (availableQuantity.compareTo(Env.ZERO) > 0) {
				// Create new Invoice Line
				MOrderLine returnOrderLine = RMAUtil.copyRMALineFromOrder(returnOrder, sourcerOrderLine,
						transactionName);
	 ...
	 * @return
	 */
	public static BigDecimal getAvailableQuantityForReturn(int sourceOrderLineId, BigDecimal sourceQuantity,
	...
	public static BigDecimal getAvailableQuantityForReturn(int sourceOrderLineId, int rmaLineId,
	...
	public static BigDecimal getReturnedQuantity(int sourceOrderLineId) {
		BigDecimal quantity = new Query(Env.getCtx(), I_C_OrderLine.Table_Name,
				ColumnsAdded.COLUMNNAME_ECA14_Source_OrderLine_ID + " = ? "
						+ "AND EXISTS(SELECT 1 FROM C_Order o WHERE o.C_Order_ID = C_OrderLine.C_Order_ID)",
				null).setParameters(sourceOrderLineId)
	...
	public static BigDecimal getReturnedQuantityExcludeRMA(int sourceOrderLineId, int rmaLineId) {
		BigDecimal quantity = new Query(Env.getCtx(), I_C_OrderLine.Table_Name,
				ColumnsAdded.COLUMNNAME_ECA14_Source_OrderLine_ID + " = ? AND C_OrderLine_ID <> ? "
						+ "AND EXISTS(SELECT 1 FROM C_Order o WHERE o.C_Order_ID = C_OrderLine.C_Order_ID)",
				null).setParameters(sourceOrderLineId, rmaLineId)
				.aggregate(I_C_OrderLine.COLUMNNAME_QtyEntered, Query.AGGREGATE_SUM);
		return Optional.ofNullable(quantity).orElse(Env.ZERO);
	}
	...
	public static void deleteRMALine(int rmaLineId) {
		...
		Trx.run(transactionName -> {
			MOrderLine rmaLine = new Query(Env.getCtx(), I_C_OrderLine.Table_Name,
					I_C_OrderLine.COLUMNNAME_C_OrderLine_ID + " = ?", transactionName).setParameters(rmaLineId)
					.setClient_ID().first();
			if (rmaLine != null && rmaLine.getC_OrderLine_ID() > 0) {
	...
	public static void deleteRMA(int rmaId) {
		...
		Trx.run(transactionName -> {
			MOrder rmaLine = new Query(Env.getCtx(), I_C_Order.Table_Name, I_C_Order.COLUMNNAME_C_Order_ID + " = ?",
					transactionName).setParameters(rmaId).setClient_ID().first();
			if (rmaLine != null && rmaLine.getC_Order_ID() > 0) {
				// Validate processed Order
				if (rmaLine.isProcessed()) {
