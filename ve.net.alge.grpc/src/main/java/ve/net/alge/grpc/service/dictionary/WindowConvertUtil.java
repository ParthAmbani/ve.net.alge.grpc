...
import ve.net.alge.grpc.core.domains.models.MADFieldCondition;
import ve.net.alge.grpc.core.domains.models.MADFieldDefinition;
import ve.net.alge.grpc.dictionary.custom.FieldCustomUtil;
...
public class WindowConvertUtil {
		...
		if(withTabs) {
			...
			if (tabs != null) {
				for(MTab tab : tabs) {
					...
					// 		tabFieldGroup.setUuid(tabFieldGroup.get_UUID() + "---");
					// 		//	Add to list
	...
	public static Table.Builder convertTable(MTable table) {
		...
		List<String> selectionColums = columnsList.stream()
			.filter(column -> {
		...
		;
		List<String> identifierColumns = columnsList.stream()
			.filter(column -> {
...
//			.setIsAllowCopy(field.isAllowCopy())// TODO core
...
//			.setIsRange(column.isRange())// TODO core
		...
		if (fieldCustom != null && fieldCustom.isActive()) {
			...
			if (fieldCustom.get_ColumnIndex(ve.net.alge.grpc.dictionary.util.DictionaryUtil.IS_DISPLAYED_AS_PANEL_COLUMN_NAME) >= 0) {
			...
			if (fieldCustom.get_ColumnIndex(ve.net.alge.grpc.dictionary.util.DictionaryUtil.IS_DISPLAYED_AS_TABLE_COLUMN_NAME) >= 0) {
	...
	public static List<DependentField> generateDependentFields(MField field) {
		...
		MTab parentTab = MTab.get(field.getAD_Tab_ID());
