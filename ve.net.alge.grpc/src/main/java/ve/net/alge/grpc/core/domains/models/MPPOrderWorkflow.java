...
public class MPPOrderWorkflow extends X_PP_Order_Workflow
{
	...
	public static MPPOrderWorkflow get (Properties ctx, int PP_Order_Workflow_ID)
	{
		if (PP_Order_Workflow_ID <= 0)
			return null;
		MPPOrderWorkflow retValue = (MPPOrderWorkflow)s_cache.get(PP_Order_Workflow_ID);
		if (retValue != null)
			return retValue;
		...
		if (node != null)
		{
			...
			for (MPPOrderNodeNext next : node.getTransitions(AD_Client_ID))
			{
				...
				if (child != null)
				{
					...
					{
						log.saveError("Error", "Cyclic transition found - "+node+" -> "+child);
					}
				}
			}
...
//			if (old.signum() != 0)
//			{
//				node.addDescription(Msg.getMsg(getCtx(), "Voided") + " (" + old + ")");
//				node.voidIt();
//				node.saveEx();
//			}
		}
