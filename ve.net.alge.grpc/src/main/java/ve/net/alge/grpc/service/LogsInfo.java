...
import ve.net.alge.grpc.log.LogsConvertUtil;
import ve.net.alge.grpc.log.LogsServiceLogic;
import ve.net.alge.grpc.util.ConvertUtil;
import ve.net.alge.grpc.util.RecordUtil;
import ve.net.alge.grpc.util.WorkflowUtil;
...
public class LogsInfo extends LogsImplBase {
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(LogsInfo.class);
	...
	private ListProcessLogsResponse.Builder listProcessLogs(ListProcessLogsRequest request) {
		...
		int[] processInstanceList = new Query(
			Env.getCtx(),
		...
		if (processInstanceList == null || processInstanceList.length <= 0) {
			return builder;
		}
...
//				.setLimit(limit, offset)
	...
	private ListRecentItemsResponse.Builder listRecentItems(ListRecentItemsRequest request) {
		...
		List<MRecentItem> recentItemsList = getFromUserAndRole(Env.getCtx());
		...
		for(MRecentItem recentItem : recentItemsList) {
			try {
				...
				if(recentItem.getAD_Tab_ID() > 0) {
					MTab tab = MTab.get(recentItem.getAD_Tab_ID());
...
//				if(recentItem.getAD_Menu_ID() > 0) {// TODO core
...
//				.setLimit(limit, offset)
	...
	public static List<MRecentItem> getFromUserAndRole(Properties ctx, int userId, int roleId, boolean isForShow) {
		List<MRecentItem> recentItems = new Query(ctx, MRecentItem.Table_Name, "AD_User_ID = ? AND AD_Role_ID = ?", null)
