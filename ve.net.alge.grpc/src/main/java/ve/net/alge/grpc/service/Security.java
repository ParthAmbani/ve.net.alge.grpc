...
import ve.net.alge.grpc.util.PreferenceUtil;
...
public class Security extends SecurityImplBase {
	...
	public Security() {
		super();
//		DB.validateSupportedUUIDFromDB();// TODO core may be not need
...
//		}
	...
	private int getUserId(String userName, String userPass) {
		...
		return 0; // login.getAuthenticatedUserId(userName, userPass);// TODO core change also need
	}
	...
	private Session.Builder runLogin(LoginRequest request, boolean isDefaultRole) {
		...
		if(!Util.isEmpty(request.getToken())) {
			org.spin.model.MADToken token = SessionManager.createSessionFromToken(request.getToken());
...
//			.setIsAllowHtmlView(role.isAllow_HTML_View()) // TODO core
...
//			.setIsAllowInfoCashJournal(role.isAllow_Info_CashJournal()) // TODO core
//			.setIsAllowInfoCrp(role.isAllow_Info_CRP()) // TODO core
			.setIsAllowInfoInOut(role.isAllow_Info_InOut())
			.setIsAllowInfoInvoice(role.isAllow_Info_Invoice())
//			.setIsAllowInfoMrp(role.isAllow_Info_MRP()) // TODO core
...
//			.setIsAllowXlsView(role.isAllow_XLS_View()) // TODO core
			.setIsShowAccounting(
				role.isShowAcct()
			)
		;
		...
		if(treeId > 0) {
			...
			while (childrens.hasMoreElements()) {
				...
				Menu.Builder childBuilder = convertMenu(
					Env.getCtx(),
					MMenu.get(Env.getCtx(), child.getNode_ID()),
					child.getParent_ID(),
					language
				);
				//	Explode child
...
//			}
	...
	private void addChildren(Properties context, Menu.Builder builder, MTreeNode node, String language) {
		...
		while (childrens.hasMoreElements()) {
			MTreeNode child = (MTreeNode) childrens.nextElement();
			MMenu menuNone = MMenu.get(context, child.getNode_ID());
			Menu.Builder childBuilder = convertMenu(
				context,
				menuNone,
				child.getParent_ID(),
