 ...
 ************************************************************************************/
package ve.net.alge.grpc.service.form.task_management;
...
public class TaskManagemetServiceLogic {
	...
	public static ListTasksResponse.Builder listTasks(ListTasksRequest request) throws ParseException {
		...
		if (request.getIsWithRequests()) {
		 ...
		 Arrays.stream(requestQuery.getIDs()).parallel().forEach(requestId -> {
		...
		if (request.getIsWithResourceAssignments()) {
			...
			Arrays.stream(resourceAssignmentQuery.getIDs()).parallel().forEach(resourceAssignmentId -> {
		...
		if (request.getIsWithProjects()) {
			...
			Arrays.stream(projectQuery.getIDs()).parallel().forEach(projectId -> {
