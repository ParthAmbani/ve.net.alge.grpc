...
import ve.net.alge.grpc.core.domains.models.I_AD_AttachmentReference;
import ve.net.alge.grpc.core.domains.models.MADAttachmentReference;
import ve.net.alge.grpc.core.domains.models.MADToken;
import ve.net.alge.grpc.core.domains.models.MADTokenDefinition;
import ve.net.alge.grpc.freight.engine.FreightEngine;
import ve.net.alge.grpc.freight.engine.FreightEngineFactory;
import ve.net.alge.grpc.store.model.MCPaymentMethod;
import ve.net.alge.grpc.store.model.MWDeliveryViaRuleAllocation;
import ve.net.alge.grpc.store.util.VueStoreFrontUtil;
import ve.net.alge.grpc.util.AttachmentUtil;
import ve.net.alge.grpc.util.DocumentUtil;
import ve.net.alge.grpc.util.TokenGeneratorHandler;
...
//		}
	...
	private MBPartnerLocation createBusinessPartnerLocation(MBPartner customer, AddressRequest address, boolean isBillTo, String transactionName) {
		...
		if(!Util.isEmpty(address.getCountryCode())) {
			MCountry country = get(Env.getCtx(), address.getCountryCode());
...
//		.setLimit(limit, offset)
	...
	private ListShippingMethodsResponse.Builder listShippingMethods(ListShippingMethodsRequest request) {
		...
		query
//		.setLimit(limit, offset)
		.<MWDeliveryViaRuleAllocation>list().forEach(shippmentMethod -> {
...
//									transactionName, 
//									null);
//					MFreightCategory freightCategory = new MFreightCategory(Env.getCtx(), shippmentMethod.getM_FreightCategory_ID(), transactionName);
...
//							.setCarrierCode(shipper.get_UUID())
//							.setCarrierName(shipper.getName())
//							.setMethodCode(shippmentMethod.get_UUID())
...
//								.setIsAvailable(true));
				...
				shippingMethodList.add(ShippingMethod.newBuilder()
					.setCarrierCode(shippmentMethod.get_UUID())
	...
	private ListPaymentMethodsResponse.Builder listPaymentMethods(ListPaymentMethodsRequest request) {
		...
		query
//		.setLimit(limit, offset)
		.<MCPaymentMethod>list().forEach(paymentMethod -> {
			builder.addPaymentMethods(
				PaymentMethod.newBuilder()
					.setCode(
						ValueManager.validateNull(
...
//		newUser.setIsLoginUser(true);// TODO core
...
//		newUser.setIsWebstoreUser(true);
	...
	private ChangePasswordResponse.Builder changePassword(ChangePasswordRequest request) {
		...
		// TODO core change for login user
...
//		}
	...
	private void sendEMail(MUser user, MADToken token) {
		...
		//	Get
// TODO core full method to check 
...
//		userMail.saveEx();
	...
	private ListProductsResponse.Builder listProducts(ListProductsRequest request) {
		...
		query
//		.setLimit(limit, offset)
		.<MProduct>list().forEach(product -> builder.addProducts(convertProduct(product, priceList, validFrom)));
		//	
		builder.setRecordCount(count);
		//	Set page token
		if(count > offset && count > limit) {
	...
	private ListRenderProductsResponse.Builder listRenderProducts(ListRenderProductsRequest request) {
		...
		query
//		.setLimit(limit, offset)
		.<MProduct>list().forEach(product -> builder.addRenderProducts(convertRenderProduct(product, priceList, validFrom)));
		//	
		builder.setRecordCount(count);
		//	Set page token
		if(count > offset && count > limit) {
...
//			.setProductGroupId(product.getM_Product_Group_ID())// TODO core
			.setCreated(
